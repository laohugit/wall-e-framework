package cn.sx.hu.wall.e.springframework.support.log;

import cn.sx.hu.wall.e.specs.constant.GlobalConstant;
import cn.sx.hu.wall.e.springframework.support.util.WebContextUtil;
...
public class LogProcessor {
    ...
    public static final String BEAN_ID = "_" + GlobalConstant.FRAMEWORK_BEAN_ID + "_LOG_PROCESSOR_";
    ...
    @Around("@within(cn.sx.hu.wall.e.specs.annotation.Log)")
    ...
    private void printParams(LogUO logUO){
        try{
            ...
            if(logConfig.isPrintParams() && Objects.nonNull(logUO.getParams()) && !(logUO.getParams().isEmpty())) {
                ...
                logger.info("{} [{}] - {} - PARAMS:{}",
                        GlobalConstant.FRAMEWORK_DISPLAY_NAME, logUO.getId(), logUO.getUrl(), joiner);
            }
        }catch (Exception ignored){
    ...
    private void printResult(LogUO logUO){
        try{
            ...
            if(logConfig.isPrintResult()) {
                logger.info("{} [{}] - {} - RESULT:{}, COSTS:{}",
                        GlobalConstant.FRAMEWORK_DISPLAY_NAME, logUO.getId(), logUO.getUrl(), stringifyObject(logUO.getResult()), logUO.getCostTimeMillis());
            }
        }catch (Exception ignored){
    ...
    private void printError(LogUO logUO){
        try{
            if(logConfig.isPrintError()) {
                ...
                error.append(GlobalConstant.FRAMEWORK_DISPLAY_NAME)
                     .append(" [")
                     .append(logUO.getId())
                     .append("]")
                     .append(" - ERROR:")
